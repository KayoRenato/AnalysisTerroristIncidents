#Problema de Negócio - Analisar Incidentes e Atos de Violência de Terrorismo
...
dados_ausente <- function(dataFrame) {
  df <- data.frame(Ausencia = round(colSums(is.na(dataFrame))/dim(dataFrame)[1]*100,2), Valor_abs = colSums(is.na(dataFrame))) 
  View(df)
  return(df)}
